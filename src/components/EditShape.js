...
const EditShape = (props) => {
    ...
    const handleSubmit = (event) => {
        ...
        if (correct) {
            ...
            id ? updateShape(id, data).then(success).catch(error) : createShape(data).then(success).catch(error)
        }
    }
    ...
    return (
        <Modal show={show} onHide={() => setShow(false)}>
            <Modal.Header closeButton>
                <Modal.Title>{id ? 'Редактирование' : 'Создание'} формы корпуса</Modal.Title>
            </Modal.Header>
            <Modal.Body>
